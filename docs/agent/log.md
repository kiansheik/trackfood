...
- Separated barcode identity from barcode nutrition. Food review can now use a scanned/manual barcode to fetch Open Food Facts name/brand even when the provider has no nutrition data; label-OCR/manual nutrition, basis and serving values are never overwritten by that identity lookup. Barcode capture triggers the lookup automatically, manual entry has an explicit `Find name & brand` action, generated names such as `OCR food` may be replaced, and user-entered names/brands remain authoritative.
- Added barcode identity provenance to saved source metadata and tests covering no-nutrition Open Food Facts products plus the rule that provider identity can enrich a label draft without changing any scanned nutrition.
